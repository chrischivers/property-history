...
object CatsEffectUtils {
  ...
  implicit class CatsEffectOps[F[_]: Async, T](f: F[T]){
    def withBackoffRetry(maxDelay: FiniteDuration, multiplier: Double, maxRetries: Int = 50, attemptNumber: Int = 1, errorsToHandle: Throwable => Boolean = _ => true)(implicit logger: Logger[F]): F[T] = {
      ...
      f.attempt.flatMap {
        ...
        case Left(err) if !errorsToHandle(err) => logger.error(err)(s"Error type not handled by retry mechanism") >> err.raiseError[F, T]
        case Left(err) =>
