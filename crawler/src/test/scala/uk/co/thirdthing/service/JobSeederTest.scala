...
class JobSeederTest extends munit.CatsEffectSuite:
  ...
  test("Seed the jobs successfully when there are no previous jobs in table") {
    ...
    val config =
      JobSeederConfig(jobChunkSize = 10, startingMaxListingIdForFirstRun = 100, emptyRecordsToDetermineLatest = 50)
    testWith(
  ...
  test("Seed the jobs successfully when there ARE previous jobs in table") {
    ...
    val config =
      JobSeederConfig(jobChunkSize = 10, startingMaxListingIdForFirstRun = 100, emptyRecordsToDetermineLatest = 5)
    testWith(
      initialJobs = Set(
  ...
  test(
    "Calling the seed method will not increase the existing jobs list if the API does not return any successful results "
  ) {
  ...
  def service(
    initialJobsRef: Ref[IO, Map[JobId, CrawlerJob]],
    config: JobSeederConfig,
    apiClientSuccessfulIds: Set[ListingId]
  ): IO[JobSeeder[IO]] =
    ...
    val apiClient: RightmoveApiClient[IO] = RightmoveApiClient.apply[IO](
      Client.fromHttpApp[IO](
        HttpRoutes
          .of[IO] {
            case request @ GET -> Root / "api" / "propertyDetails" :? PropertyIdMatcher(id)
                if apiClientSuccessfulIds(ListingId(id)) =>
              StaticFile
