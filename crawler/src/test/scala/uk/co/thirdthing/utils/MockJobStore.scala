...
object MockJobStore:
  ...
  def apply(initialJobsRef: Ref[IO, Map[JobId, CrawlerJob]]): IO[JobStore[IO]] =
    ...
    val jobStore = new JobStore[IO]:
      ...
      override def getLatestJob: IO[Option[Model.CrawlerJob]] =
        initialJobsRef.get.map(_.view.values.toList.sortBy(_.from.value).lastOption)

      override def getAndLock(jobId: JobId): IO[Option[CrawlerJob]] = Clock[IO].realTimeInstant.flatMap { now =>
        initialJobsRef
          .updateAndGet(
            _.updatedWith(jobId)(
              _.map(_.copy(state = Model.JobState.Pending, lastRunStarted = Some(CrawlerJob.LastRunStarted(now))))
            )
          )
          .map(_.get(jobId))
      }
